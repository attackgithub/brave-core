...
void RewardsServiceImpl::GetCountryCodes(
  ...
  std::vector<std::int32_t> limited_country_codes;
  for (const auto& limited_country : limited_countries) {
    limited_country_codes.push_back(country_codes::CountryCharsToCountryID(
        limited_country.at(0), limited_country.at(1)));
  }
  callback(limited_country_codes);
}
